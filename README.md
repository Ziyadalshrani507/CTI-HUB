...
The dashboard is the first screen analysts see when they open the platform - it brings together the most important intelligence indicators in a single view so nothing important gets missed.

At the top, five stat cards give a live count of every major tracked category: Public Domains, Internal Domains, Subdomains, Assets, and Impersonation Accounts. Each card is clickable and navigates directly to the full detail page for that category.

Below the stats, two summary panels sit side by side. The left panel shows a breakdown of suspicious domain alerts by classification type, giving the team a quick read on what kind of threats are currently active. The right panel shows the current status of tracked CVEs across the remediation pipeline - how many have been discovered, reported, confirmed not in the environment, and are actively in progress.

The **Threat Level Calculator** lets analysts compute a standardized weekly risk score using the formula `(Severity × Likelihood × Criticality) − Countermeasures`. The result is displayed as a color-coded severity label and automatically logged to a historical trend chart, so the team can track whether the overall threat level is improving or worsening week over week.

Finally, the **Task Calendar** gives analysts a built-in place to track deadlines, scheduled investigations, and reporting dates - keeping all operational planning inside the platform rather than relying on external tools.
...
One of the most common challenges in CTI is receiving an alert about a suspicious domain that closely resembles a legitimate one - and not being able to quickly confirm whether it actually belongs to the organization or not. Without a reliable reference, analysts waste time chasing domains that turn out to be internal, or worse, miss ones that are genuinely malicious.

The Public Domains, Internal Domains, and Subdomains registries solve this directly. Together they form a single source of truth for everything the organization owns. When a suspicious domain comes in, analysts can cross-reference it against these lists in seconds. If it matches - it's legitimate. If it doesn't - it gets flagged for investigation and takedown. The faster that call can be made, the faster the team can act.
...
Tracks all internal network domains used within the organization's infrastructure - intranet portals, VPN gateways, mail servers, and Active Directory endpoints. Same principle as Public Domains: when a domain appears in an alert, analysts need to immediately know whether it belongs to the internal environment or not. This registry makes that verification instant, without needing to contact IT or dig through network documentation.
...
Tracks all known subdomains under the organization's primary domains - storefronts, APIs, support portals, CDN endpoints, and more. Subdomains are one of the most commonly abused attack vectors: threat actors register look-alike subdomains or take over forgotten ones to run phishing campaigns that appear to come from a trusted source. Having a complete subdomain inventory means the team can immediately spot anything in an alert that doesn't belong.
